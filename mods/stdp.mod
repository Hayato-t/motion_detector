NEURON {
	...
	RANGE tau, e, i, d, p, dtau, ptau, verbose, learning, LR, maxWeight, minWeight, numbre, debug, forSpike, tmp, forDA, ltd
...
PARAMETER {
	...
	e = 0	(mV)
	d = 1 <0,1>:kokode kaezuni sitade 1.2 bai sitemimasu: depression factor (multiplicative to prevent < 0)
	...
	verbose = 0
	learning = 1
	LR = 0.00025:0.0005:0.0003:0.0008:0.0005:0.00025:0.02:0.0001
	maxWeight = 1
	minWeight = 0
	numbre = 0
	debug = 1
...
NET_RECEIVE(w (uS), tpre (ms)) {
	...
	if(learning) {
		...
		:printf("%g \n",debug)
		w = w-(1+forDA)*LR*d*ltd*exp((tpost - t)/dtau)
		:printf("after w = %g\n",w)
		if(w<=minWeight){
		    w=minWeight
		}
